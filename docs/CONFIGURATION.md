...
Set **`VITE_DISABLE_GOOGLE_FONTS=true`** to skip Google Fonts preconnect and stylesheet injection on **public marketing routes only** (`/`, `/services`, `/portfolio`, `/about`, `/pricing`, `/contact`, `/blog`) when typography mode is **system**. Admin/dashboard routes (`/…/admin/…`) always load fonts regardless of this flag.
