...
export const useFooterLayout = routeLoader$(async ({ params, cookie, request, fail }) => {
  if (params.id === 'new') {
  ...
  try {
    ...
    const res = await api.get(API_ENDPOINTS.APPEARANCE.FOOTER_GET(id));
    ...
    return fail(404, { message: 'Footer not found' });
  }
});

export default component$(() => {
  const layout = useFooterLayout();
  return <ChromeLayoutEditPage kind="footer" layout={layout.value as ChromeLayoutMeta} />;
});

export const head: DocumentHead = { title: 'Edit footer' };
