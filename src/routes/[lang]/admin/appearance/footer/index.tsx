...
export const useFootersList = routeLoader$(async ({ cookie, request, params }) => {
  try {
    const api = adminApiClient(cookie, request, params.lang);
    const res = await api.get(API_ENDPOINTS.APPEARANCE.FOOTERS);
...
export default component$(() => {
  const list = useFootersList();
  return <ChromeLayoutListPage kind="footer" initialItems={list.value} />;
});
