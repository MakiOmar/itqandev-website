...
export const useLoopItemsList = routeLoader$(async ({ cookie, request, params }) => {
  try {
    const api = adminApiClient(cookie, request, params.lang);
    const res = await api.get(API_ENDPOINTS.APPEARANCE.LOOP_ITEMS);
...
export default component$(() => {
  const list = useLoopItemsList();
  return <ChromeLayoutListPage kind="loop_item" initialItems={list.value} />;
});

export const head: DocumentHead = { title: 'Loop item templates' };
