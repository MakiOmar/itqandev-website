...
export const useLoopItemLayout = routeLoader$(async ({ params, cookie, request, fail }) => {
  if (params.id === 'new') {
  ...
  try {
    ...
    const res = await api.get(API_ENDPOINTS.APPEARANCE.LOOP_ITEM_GET(id));
...
export default component$(() => {
  const layout = useLoopItemLayout();
  return <ChromeLayoutEditPage kind="loop_item" layout={layout.value as ChromeLayoutMeta} />;
});

export const head: DocumentHead = { title: 'Edit loop item' };
