...
export const useSingleLayout = routeLoader$(async ({ params, cookie, request, fail }) => {
  if (params.id === 'new') {
  ...
  try {
    ...
    const res = await api.get(API_ENDPOINTS.APPEARANCE.SINGLE_GET(id));
...
export default component$(() => {
  const layout = useSingleLayout();
  return <ChromeLayoutEditPage kind="single" layout={layout.value as ChromeLayoutMeta} />;
});

export const head: DocumentHead = { title: 'Edit single template' };
