...
export const useBodiesList = routeLoader$(async ({ cookie, request, params }) => {
  try {
    const api = adminApiClient(cookie, request, params.lang);
    const res = await api.get(API_ENDPOINTS.APPEARANCE.BODIES);
...
export default component$(() => {
  const list = useBodiesList();
  return <ChromeLayoutListPage kind="body" initialItems={list.value} />;
});
