...
export const useOverlaysList = routeLoader$(async ({ cookie, request, params }) => {
  try {
    const api = adminApiClient(cookie, request, params.lang);
    const res = await api.get(API_ENDPOINTS.APPEARANCE.OVERLAYS);
...
export default component$(() => {
  const list = useOverlaysList();
  return <ChromeLayoutListPage kind="overlay" initialItems={list.value} />;
});

export const head: DocumentHead = { title: 'Overlays' };
