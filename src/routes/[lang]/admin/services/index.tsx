...
export default component$(() => {
  ...
  const translations = {
    ...
    deleteConfirm: translateApp(lang, 'services.deleteConfirm'),
  };
  ...
  const translationsLabel = (svc: AdminService): string => {
    ...
    const locales = Array.isArray(rows)
      ...
      : [];
    if (locales.length === 0) {
      return String(translateApp(lang, 'contentTranslations.noSecondaryLanguages') || '—');
    }
  ...
  return (
    <>
      ...
      <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-800">
        <h2 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">{translateApp(lang, 'services.list')}</h2>
        <div class="mb-4">
          <input
            type="text"
        ...
        {loading.value ? (
          ...
          <EmptyState title={translateApp(lang, 'services.empty')} />
        ) : (
          <ul class="space-y-2">
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                      {translateApp(lang, 'services.slug')}: {svc.slug}
                      {svc.icon ? ` · ${translateApp(lang, 'services.icon')}: ${svc.icon}` : ''}
                    </p>
                    ...
                    <div class="mt-1 flex flex-wrap gap-1">
                      <span class="rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-700 dark:bg-slate-700/30 dark:text-slate-200">
                        {translateApp(lang, 'contentTranslations.contentPrimaryLanguage')}: {mainLocaleLabel(svc)}
                      </span>
                      <span class="rounded bg-emerald-100 px-2 py-0.5 text-xs text-emerald-700 dark:bg-emerald-700/20 dark:text-emerald-300">
                        {translateApp(lang, 'contentTranslations.addTranslations')}: {translationsLabel(svc)}
                      </span>
                    </div>
                    {!svc.is_published && (
                      <span class="mt-1 inline-block rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-900 dark:bg-amber-900/30 dark:text-amber-200">
                        {translateApp(lang, 'services.draft')}
                      </span>
                    )}
                  </div>
                </div>
