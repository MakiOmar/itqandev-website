...
export const useAdminAuth = routeLoader$(async ({ cookie, url, redirect: redirectFn }) => {
  ...
  // Check for login page with or without trailing slash (logical path ignores `/en` / `/ar`)
  const isLoginPage =
...
export default component$(() => {
  ...
  const normalizedPath = pathname.replace(/\/+$/, '') || '/';
  const logicalPath = stripUiLocaleFromPathname(normalizedPath);
  const isLoginPage = logicalPath === config.routes.admin.login || logicalPath === '/admin/login';
